...
public class Insert_Interval {
	...
	public static List<Interval> insert(List<Interval> intervals, Interval newInterval) {
        ...
        if (newInterval.end < intervals.get(0).start) { // if newInterval is added to the head
            list.add(newInterval);
            list.addAll(intervals);
            return list;
        }
        if (newInterval.start > intervals.get(intervals.size() - 1).end) { // if newInterval is added to the tail
            list.addAll(intervals);
            list.add(newInterval);
            return list;
        }
        ...
        int nstart = newInterval.start, nend = newInterval.end; // new interval's start and end
